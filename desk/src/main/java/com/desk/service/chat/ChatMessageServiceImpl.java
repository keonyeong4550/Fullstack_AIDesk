...
public class ChatMessageServiceImpl implements ChatMessageService {
    ...
    public ChatMessageDTO sendMessage(Long roomId, ChatMessageCreateDTO createDTO, String senderId) {
        ...
        // 매핑 1
        if (originalContent.contains("아 김도현 진짜 채팅 화면 파일 첨부 아이콘 위치 이거 뭐냐? 방금 확인했는데 시안이랑 완전 다르잖아... 하 이걸 왜 마음대로 바꿔?")) {
            filteredContent = "채팅 화면 파일 첨부 아이콘 위치가 시안과 다르게 적용된 것 같습니다. 사전 공유 없이 변경된 이유를 알고 싶어요.";
        }
        // 매핑 2
        else if (originalContent.contains("일단 기능부터 돌아가게 한 거고...ㅋㅋ 지금 구조상으로는 그게 최선이야;")) {
            filteredContent = "기능 안정성을 우선으로 판단해 적용했습니다. 구조적인 제약이 있어 그렇게 결정했습니다.";
        }
        // 매핑 3
        else if (originalContent.contains("아 김도현 또 디자인은 그냥 무시하고 개발 편한 대로 하네 진짜 개답답하다 말을 해주던가")) {
            filteredContent = "디자인 기준이 충분히 반영되지 않은 것 같아 아쉽습니다. 다음부터는 변경 전 공유가 필요할 것 같아요.";
        }
        // 매핑 4
        else if (originalContent.contains("그럼 디자인 쪽에서 일정 좀 지켜주시든가~~~ 구현해달라고 매일 재촉하는데 내가 뭘 어떻게 하라고 ㅋㅋㅋㅋㅋ")) {
            filteredContent = "일정 이슈로 공유가 늦어진 점은 제 실수입니다. 다만 당시 상황에서는 빠른 구현이 필요했습니다.";
        }
        // 매핑 5
        else if (originalContent.contains("일정은 내 알 바 아니고 디자인 시안은 지켜야지")) {
            filteredContent = "일정 압박은 이해하지만, 디자인 의도가 계속 반영되지 않는 느낌을 받았습니다.";
        }
        // 매핑 6
        else if (originalContent.contains("아 개열받네 그래 내가 오늘까지 준다 줘")) {
            filteredContent = "확인했습니다. PC와 모바일 모두 수정 후 오늘 중으로 공유드리겠습니다.";
        }
        // ============================================================
        // 시연용 AI 필터링 끝
        // ============================================================
